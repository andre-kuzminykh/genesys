...
| FR-FORECAST-001 | `POST /api/forecast` MUST persist the latest simulation result to `forecast.json` next to `state.json`. The endpoint is intentionally anonymous so the public demo can run a simulation without logging in; non-object / array bodies are rejected with 400 BAD_BODY. Accepts up to 1 MB. | TEST-FR-FORECAST-001-S |
| FR-LB-LLM-001   | The browser simulation MUST route every OpenAI call through the same-origin `/api/llm/chat` and `/api/llm/responses` proxies — no `Authorization: Bearer` header may be sent from the browser to `api.openai.com` directly. This keeps the OpenAI key out of the public JS bundle and lets ops rotate it without rebuilding `web`. | TEST-FR-LB-003-C |
| FR-LLM-DISABLE-001 | `OPENAI_API_KEY` MUST be treated as "no LLM" when its env value is empty or matches one of the sentinels `none` / `off` / `mock` / `disabled` (case-insensitive). `/auth/health` then reports `openai.hasKey:false`, every `/api/llm/*` proxy returns `503 NO_OPENAI_KEY` without calling OpenAI, and `Leaderboard.tsx` constructs `MockLlmAdapter` directly — no "LLM call failed" banner. This is the default production posture so the key never appears in any artifact (no .env value, no JS bundle field). | TEST-FR-LLM-DISABLE-001-S |
| FR-FORECAST-002 | `GET /api/forecast` MUST return the stored forecast unchanged (200) or `{ ok:false, error:'NO_FORECAST' }` (404) when no run has been recorded. Public; no auth required. | TEST-FR-FORECAST-002-S |
| FR-FORECAST-003 | On mount, `Leaderboard.tsx` MUST hydrate from `GET /api/forecast` when — and only when — the browser has no cached run under `genesys:forecast:v1`. A 200 is written through to the local cache and revealed in full (no month-by-month replay); a 404 or network error leaves the empty state untouched. A browser that already has a local run MUST NOT issue the request, so a visitor's own simulation is never overwritten by someone else's. | TEST-FR-FORECAST-003-C |
...
| v0.2.3  | 2026-05-23 | **FR-FORECAST-001/002 added**: new admin-only `POST /api/forecast` + public `GET /api/forecast`, with `Leaderboard.tsx` mirroring each simulation result server-side after `saveCached`. `scripts/scoreboard.js` now auto-detects `forecast.json` next to `state.json`, so `node scripts/scoreboard.js` on the VM prints the revenue-weighted investor leaderboard with no manual file shuffling after andre-kuzminykh runs the simulation in the browser. Covered by `TEST-FR-FORECAST-001-S` (admin gate) and `TEST-FR-FORECAST-002-S` (empty-store 404). | UPDATED |
| v0.2.4  | 2026-05-23 | **FR-LB-LLM-001 added; FR-FORECAST-001 relaxed**. The browser simulation no longer talks to `api.openai.com` directly — `OpenAILlmAdapter.chat()` now POSTs to the same-origin `/api/llm/chat` proxy (same path `chatWithSearch` already used). `BAKED_OPENAI_KEY` is gone from the JS bundle: rotating the key is a server-side `.env` edit + `docker compose up -d --force-recreate auth`, no web rebuild. `/api/forecast` POST dropped its admin gate (still rejects non-object bodies) so an anonymous demo visitor can run the simulation end-to-end without GitHub login. Covered by `TEST-FR-LB-003-C` (browser proxy + no Authorization header) and the updated `TEST-FR-FORECAST-001-S`. | UPDATED |
| v0.2.5  | 2026-05-23 | **FR-LLM-DISABLE-001 added**: empty / `none` / `off` / `mock` / `disabled` (case-insensitive) `OPENAI_API_KEY` now disables the LLM proxy at boot. `/auth/health` reports `hasKey:false`, every `/api/llm/*` route returns `503 NO_OPENAI_KEY` without hitting OpenAI, and `Leaderboard.tsx` short-circuits to `MockLlmAdapter` (logs "Running deterministic simulation"). Operator can now keep the key out of every artifact entirely — set `OPENAI_API_KEY=none` in `/opt/genesis/.env` and the whole stack runs deterministic. Covered by `TEST-FR-LLM-DISABLE-001-S`. | UPDATED |
| v0.2.6  | 2026-09-11 | **FR-FORECAST-003 added**: `Leaderboard.tsx` now hydrates from `GET /api/forecast` on mount when the browser has no cached run, closing the gap where a server-side forecast was readable by `scripts/scoreboard.js` but invisible in the UI until somebody pressed Run. A browser with its own local run skips the request entirely, so a visitor's simulation is never clobbered. Also adds `scripts/seed-demo-data.js` — a deterministic seeder that fills `state.json` + `forecast.json` with a plausible play-through (upvotes, investments, 13-month forecast) for screenshots and local demos; it enforces the same invariants as the live API (wallet cap, no self-invest, one vote per handle). Covered by `TEST-FR-FORECAST-003-C`. | UPDATED |
