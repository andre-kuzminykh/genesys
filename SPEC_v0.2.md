...
| FR-FORECAST-001 | `POST /api/forecast` MUST persist the latest admin-driven simulation result to `forecast.json` next to `state.json`. The endpoint MUST reject non-admin callers with 403 NOT_ADMIN, and missing-token callers with 401 NO_TOKEN. Accepts up to 1 MB. | TEST-FR-FORECAST-001-S |
